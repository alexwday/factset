...
class SentenceAligner:
    """Advanced sliding window sentence alignment with word-level precision."""
    ...
    def __init__(self, similarity_threshold: float = 0.7, window_size: int = 4, min_match_length: int = 3):
        ...
        self.word_matches = []  # Store word-level matches from sliding window
    ...
    def _merge_word_matches(self, matches: List[Dict]) -> List[Dict]:
        ...
        # Sort by PDF sentence and word position
        ...
        for current in matches[1:]:
            ...
            # Check for overlap in same sentence
    ...
    def _build_sentence_alignments_from_word_matches(self, pdf_indexed: List[IndexedSentence], 
        ...
        # Group word matches by sentence pairs
        ...
        # Create sentence matches from grouped word matches
    ...
    def _handle_unmatched_sentences(self, pdf_indexed: List[IndexedSentence], html_indexed: List[IndexedSentence]):
        ...
        print("    - Handling unmatched sentences with fallback matching...")
        
        # Try direct similarity matching for unmatched sentences
        unmatched_pdf = [s for s in pdf_indexed if not s.is_matched]
        unmatched_html = [s for s in html_indexed if not s.is_matched]
        
        for pdf_sent in unmatched_pdf:
            best_match = None
            best_similarity = 0.0
            
            for html_sent in unmatched_html:
                if html_sent.is_matched:
                    continue
                
                similarity = self._calculate_sentence_similarity(pdf_sent.text, html_sent.text)
                
                if similarity > best_similarity and similarity >= 0.6:  # Lower threshold for fallback
                    best_match = html_sent
                    best_similarity = similarity
            
            if best_match:
                match_id = f"fallback_{self.match_counter}"
                self.match_counter += 1
                ...
                self.matches.append(SentenceMatch(
                    ...
                    match_type='fallback_similarity',
    ...
    def _convert_to_position_ordered_format(self, pdf_indexed: List[IndexedSentence], 
        ...
        # First pass: Process all sentences in their original order
        # We'll go through each position and handle whatever we find there
            ...
            if pdf_sent.is_matched and pdf_idx not in pdf_processed:
                ...
                for match in self.matches:
                    if any(s.index == pdf_idx for s in match.pdf_sentences):
                        # This is a match involving this PDF sentence
        ...
        for html_idx in range(len(html_indexed)):
            if html_idx not in html_processed:
                ...
                # This HTML sentence wasn't part of any match
                alignments.append(SentenceAlignment(
        ...
        # Now we need to sort the alignments to maintain visual order
        # Create a position key for each alignment based on the minimum index involved
        def get_position_key(alignment):
            pdf_indices = []
            html_indices = []
            
            # Get indices for sentences in this alignment
            ...
            # Use the minimum index from either side for positioning
            pdf_min = min(pdf_indices) if pdf_indices else float('inf')
            html_min = min(html_indices) if html_indices else float('inf')
            
            # For DELETE, use PDF index; for INSERT, use HTML index; for matches, use minimum
