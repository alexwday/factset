"""
Stage 1: Daily Transcript Sync with Invalid Transcript Tracking
Downloads recent earnings transcripts from FactSet API with invalid transcript filtering.
        ...
        try:
            ...
            for transcript in all_transcripts:
                ...
                if len(primary_ids) == 1 and primary_ids[0] in monitored_tickers:
                    # Add ALL transcripts for our monitored tickers (filtering happens during download)
...
def compare_transcripts_with_invalid_list(
            ...
            if event_id in nas_by_event_id:
                ...
                if matching_nas:
                    ...
                    if api_transcript["version_id"] != matching_nas["version_id"]:
                        ...
                        # For daily sync, we can remove old versions
    ...
    log_execution(
        "Daily sync transcript comparison completed (with invalid list check)",
        {
            ...
            "comparison_logic": "Check invalid list first, version management for updates",
        },
    )
...
def main() -> None:
    ...
    log_console("=== STAGE 1: DAILY TRANSCRIPT SYNC (WITH INVALID TRACKING) ===")
    log_execution(
    ...
    try:
        ...
        with fds.sdk.EventsandTranscripts.ApiClient(api_configuration) as api_client:
            ...
            if not banks_with_transcripts:
                ...
                for i, ticker in enumerate(banks_with_transcripts, 1):
                    ...
                    to_download, to_remove = compare_transcripts_with_invalid_list(
        ...
        if institutions_with_transcripts:
            log_console(f"Daily sync complete: {total_downloaded} downloaded, {total_added_to_invalid} added to invalid list")
